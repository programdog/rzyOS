...
#define SET_SHOWPLUS(f)       \
	do                        \
		(f) |= FLAG_SHOWPLUS; \
	while (0)
#define SET_ALTFORM(f)       \
	do                       \
		(f) |= FLAG_ALTFORM; \
	while (0)
#define SET_HASDOT(f)       \
	do                      \
		(f) |= FLAG_HASDOT; \
	while (0)
#define SET_HASASTERISKWIDTH(f)       \
	do                                \
		(f) |= FLAG_HASASTERISKWIDTH; \
	while (0)
#define SET_HASASTERISKTRUNC(f)       \
	do                                \
		(f) |= FLAG_HASASTERISKTRUNC; \
	while (0)
#define SET_LONGPRECISION(f)       \
	do                             \
		(f) |= FLAG_LONGPRECISION; \
	while (0)
#define SET_LONGLONGPRECISION(f)       \
	do                                 \
		(f) |= FLAG_LONGLONGPRECISION; \
	while (0)
#define SET_NEGATE(f)       \
	do                      \
		(f) |= FLAG_NEGATE; \
...
#define CLR_SHOWPLUS(f)        \
	do                         \
		(f) &= ~FLAG_SHOWPLUS; \
	while (0)
#define CLR_ALTFORM(f)        \
	do                        \
		(f) &= ~FLAG_ALTFORM; \
	while (0)
#define CLR_HASDOT(f)        \
	do                       \
		(f) &= ~FLAG_HASDOT; \
	while (0)
#define CLR_HASASTERISKWIDTH(f)        \
	do                                 \
		(f) &= ~FLAG_HASASTERISKWIDTH; \
	while (0)
#define CLR_HASASTERISKTRUNC(f)        \
	do                                 \
		(f) &= ~FLAG_HASASTERISKTRUNC; \
	while (0)
#define CLR_LONGPRECISION(f)        \
	do                              \
		(f) &= ~FLAG_LONGPRECISION; \
	while (0)
#define CLR_LONGLONGPRECISION(f)        \
	do                                  \
		(f) &= ~FLAG_LONGLONGPRECISION; \
	while (0)
#define CLR_NEGATE(f)        \
...
#define FMT_NEXT \
	src++;       \
	ch = up_romgetc(src) /* Advance to the next character */
#define FMT_PREV \
	src--;       \
	ch = up_romgetc(src) /* Backup to the previous character */
#else
...
int lib_vsprintf(FAR struct lib_outstream_s *obj, FAR const char *src,
				 va_list ap)
{
	FAR char *ptmp;
#ifndef CONFIG_NOPRINTF_FIELDWIDTH
